...
<body class="text-gray-800 font-inter">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                <div class="bg-white border border-gray-100 shadow-md shadow-black/5 p-6 rounded-md lg:col-span-2 w-full">
                    <div class="flex justify-between mb-4 items-start">
                        <!-- <label for="role" class="block text-sm font-medium text-gray-600"></label> -->
                            <select id="role" name="category" class="mt-1 p-2 border rounded-md focus:outline-none focus:border-blue-500" onchange="getUserData()">
    ...
    <script>
        function getUserData() {
            ...
            xhr.open('POST', 'get_users.php');
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xhr.onload = function() {
                if (xhr.status === 200) {
                    document.querySelector('.all-user tbody').innerHTML = xhr.responseText;
                }
            };
            xhr.send('role=' + role);
        }
